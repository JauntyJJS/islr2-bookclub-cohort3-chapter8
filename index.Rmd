...
t0 predict only .blue[**one**] value: $\hat{f}^1_k(x) = \frac{1}{nK}\sum_{i=1}^{n}y_i$
, the .blue[**mean**] of the response values divided by the total number of trees. Thus
...
The other $K-1$ trees include those that have already been updated by BART and those that will be updated by BART later.
...
Changes that greatly improve the fit to the partial residual will be given a .blue[**higher probability**]. However, penalty scores will be given when adding new branches to prevent the tree from growing too deep. Recall that unlike boosting, we do no have control on the depth
]
